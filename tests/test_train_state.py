...
def test_no_repetition_preflight_rejects_epoch_rollover(tmp_path) -> None:
    values = np.arange(32, dtype=np.uint32)
    (tmp_path / "train.bin").write_bytes(values.tobytes())
    config = _make_config(str(tmp_path))
    config.dataset.allow_repetition = False
    config.trainer.max_steps = 5

    with pytest.raises(RuntimeError, match="required_blocks=10, available_blocks=8"):
        preflight_training_data(config, object())


def test_wsd_schedule_has_warmup_stable_and_decay_phases() -> None:
    config = _make_config("unused")
    config.trainer.max_steps = 10
    config.trainer.warmup_steps = 2
    config.trainer.lr_schedule = "wsd"
    config.trainer.decay_fraction = 0.2
    config.trainer.min_lr_ratio = 0.0

    assert learning_rate_multiplier(0, config) == 0.5
    assert learning_rate_multiplier(1, config) == 1.0
    assert learning_rate_multiplier(2, config) == 1.0
    assert learning_rate_multiplier(8, config) == 1.0
    assert learning_rate_multiplier(9, config) == 0.5
    assert learning_rate_multiplier(10, config) == 0.0

...
run_dir=$project_root/runs/l20-edu-135m-fineweb-pilot-50m
telemetry_path=$project_root/remote_logs/fineweb-pilot-telemetry-${SLURM_JOB_ID:-manual}.csv
...
if [[ -e "$run_dir" ]]; then
  echo "refusing to overwrite existing pilot run: $run_dir" >&2
  exit 3
fi
...
cd "$project_root"
echo "execution_scope=50m_real_data_pilot_not_final_quality_evidence"
...
"$runtime_python" -m l20_pretrain.train \
  configs/l20_135m_fineweb_pilot_50m.yaml \
  --device cuda

...
run_dir=$project_root/runs/l20-edu-140m-wide-fineweb-wsd-pilot-50m
telemetry_path=$project_root/remote_logs/wide-fineweb-wsd-pilot-telemetry-${SLURM_JOB_ID:-manual}.csv
...
if [[ -e "$run_dir" ]]; then
  echo "refusing to overwrite existing wide WSD pilot run: $run_dir" >&2
  exit 3
fi
...
cd "$project_root"
echo "execution_scope=matched_50m_wide_wsd_factorial_cell_not_final_quality_evidence"
...
"$runtime_python" -m l20_pretrain.train \
  configs/l20_140m_wide_fineweb_wsd_pilot_50m.yaml \
  --device cuda
...
echo "wide_wsd_pilot_status=pass"

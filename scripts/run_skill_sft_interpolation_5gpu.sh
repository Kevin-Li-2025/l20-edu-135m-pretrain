...
PILOT_ROOT="${PILOT_ROOT:-${PROJECT_ROOT}/runs/posttrain/skill-curriculum-sft-lr-pilot-v1}"
RUN_ROOT="${RUN_ROOT:-${PROJECT_ROOT}/runs/posttrain/skill-curriculum-sft-interpolation-v1}"
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/eval_results/posttrain/skill-curriculum-sft-interpolation-v1}"
...
names=(lr5e5-a025 lr5e5-a050 lr5e5-a075 lr1e4-a025 lr1e4-a050)
tuned_names=(lr5e5 lr5e5 lr5e5 lr1e4 lr1e4)
alphas=(0.25 0.50 0.75 0.25 0.50)
...
for index in 0 1 2 3 4; do
  ...
  if [[ ! -s "${model}/model.safetensors" ]]; then
    "${PYTHON_BIN}" "${PROJECT_ROOT}/scripts/interpolate_models.py" \
      --base "${BASE}" \
      --tuned "${PILOT_ROOT}/${tuned_names[$index]}/step-000200" \
      --alpha "${alphas[$index]}" \
      --output "${model}"
  fi
done
...
if (( status != 0 )); then
  printf 'one or more skill interpolation evaluations failed under %s\n' "${EVAL_ROOT}" >&2
  exit "${status}"
fi
...
printf 'skill SFT interpolation evaluation complete: %s\n' "${EVAL_ROOT}"

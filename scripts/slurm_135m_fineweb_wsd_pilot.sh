...
run_dir=$project_root/runs/l20-edu-135m-fineweb-wsd-pilot-50m
telemetry_path=$project_root/remote_logs/fineweb-wsd-pilot-telemetry-${SLURM_JOB_ID:-manual}.csv
...
if [[ -e "$run_dir" ]]; then
  echo "refusing to overwrite existing WSD pilot run: $run_dir" >&2
  exit 3
fi
...
cd "$project_root"
echo "execution_scope=matched_50m_wsd_ablation_not_final_quality_evidence"
...
"$runtime_python" -m l20_pretrain.train \
  configs/l20_135m_fineweb_wsd_pilot_50m.yaml \
  --device cuda
